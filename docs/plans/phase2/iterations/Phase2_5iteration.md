...
> Текущее состояние реализации: источник `(outpoint → anchor)` ещё не подключён, поэтому кэш `StealthAnchorHintCache` остаётся пустым и `anchor_hint` в ответах `get_block_view_tags` всегда `None`. Это влияет только на производительность лёгких клиентов (нет раннего отсечения по anchor), функциональность не ломает. Добавим заполнение кэша, когда появится надёжный источник якорей на ноде.
