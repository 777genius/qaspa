...
fn get_sig_op_count_by_opcodes<T: VerifiableTransaction, Reused: SigHashReusedValues>(
    ...
    for (i, op) in opcodes.iter().enumerate() {
        match op {
            Ok(op) => {
                match op.value() {
                    codes::OpCheckSig | codes::OpCheckSigVerify | codes::OpCheckSigECDSA | codes::OpCheckSigMLDSA => num_sigs += 1,
    ...
    fn check_schnorr_signature(&mut self, hash_type: SigHashType, key: &[u8], sig: &[u8]) -> Result<bool, TxScriptError> {
        ...
        match self.script_source {
            ScriptSource::TxInput { tx, idx, .. } => {
                ...
                let msg =
                    secp256k1::Message::from_digest_slice(sig_hash.as_bytes().as_slice()).map_err(TxScriptError::InvalidSignature)?;
                let sig_cache_key =
