...
/// Compounded subscription for BlockAdded notifications with optional stealth outputs
#[derive(Clone, Default, Debug, PartialEq, Eq)]
pub struct BlockAddedSubscription {
    include_stealth_outputs: [usize; 2],
}

impl BlockAddedSubscription {
    #[inline(always)]
    fn all(&self) -> usize {
        self.include_stealth_outputs[true as usize]
    }

    #[inline(always)]
    fn all_mut(&mut self) -> &mut usize {
        &mut self.include_stealth_outputs[true as usize]
    }

    #[inline(always)]
    fn reduced(&self) -> usize {
        self.include_stealth_outputs[false as usize]
    }

    #[inline(always)]
    fn reduced_mut(&mut self) -> &mut usize {
        &mut self.include_stealth_outputs[false as usize]
    }
}
...
impl Compounded for BlockAddedSubscription {
    fn compound(&mut self, mutation: Mutation, _context: &SubscriptionContext) -> Option<Mutation> {
        ...
        if let Scope::BlockAdded(ref scope) = mutation.scope {
            let all = scope.include_stealth_outputs;
            match mutation.command {
                Command::Start => {
                    if all {
                        // Add All (with stealth)
                        *self.all_mut() += 1;
                        if self.all() == 1 {
                            return Some(mutation);
                        }
                    } else {
                        // Add Reduced (no stealth)
                        *self.reduced_mut() += 1;
                        if self.reduced() == 1 && self.all() == 0 {
                            return Some(mutation);
                        }
...
impl Subscription for BlockAddedSubscription {
    ...
    fn active(&self) -> bool {
        self.include_stealth_outputs.iter().sum::<usize>() > 0
    }
...
mod tests {
    ...
    use crate::{
        ...
        scope::SinkBlueScoreChangedScope,
    };
    ...
    fn test_overall_compounding() {
        let none = || Box::new(OverallSubscription::new(EventType::SinkBlueScoreChanged));
        let add = || Mutation::new(Command::Start, Scope::SinkBlueScoreChanged(SinkBlueScoreChangedScope {}));
        let remove = || Mutation::new(Command::Stop, Scope::SinkBlueScoreChanged(SinkBlueScoreChangedScope {}));
        let test = Test {
