...
pub fn sign_with_multiple_v2(mut mutable_tx: SignableTransaction, privkeys: &[[u8; 32]]) -> Signed {
    let mut map = BTreeMap::new();
    for privkey in privkeys {
        if let Ok(schnorr_key) = secp256k1::Keypair::from_seckey_slice(secp256k1::SECP256K1, privkey) {
...
pub fn sign_input(tx: &impl VerifiableTransaction, input_index: usize, private_key: &[u8; 32], hash_type: SigHashType) -> Vec<u8> {
    try_sign_input(tx, input_index, private_key, hash_type).unwrap_or_default()
}

/// Sign a transaction input with a sighash_type using schnorr (fallible).
